...
# Load actual data
actual_data = np.genfromtxt('actual_data_hippo_V14_1.csv', delimiter=',', skip_header=1)
...
for dim in range(3):
    ...
    plt.plot(actual_dim_data, label=f'Actual {chr(88 + dim)}', color='r', lw=0.8)

    # label=f'Predicted {chr(88 + dim)} (Dataset {i + 1})'
    
    # Loop over each predicted dataset
    for i in range(n_predicted_datasets):
        # Load predicted data for the current dataset
        predicted_data = np.genfromtxt(f'predicted_data_hippo_V14_{i}.csv', delimiter=',', skip_header=1)
